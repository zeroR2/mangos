    ...
    switch (targetMode)
    {
        ...
        case TARGET_RANDOM_ENEMY_CHAIN_IN_AREA:
        {
            ...
            while(t && next != tempTargetUnitMap.end())
            {
                ...
                if((!prev->IsWithinLOSInMap(*next, false) && !(m_spellInfo->AttributesEx2 & SPELL_ATTR_EX2_IGNORE_LOS)) || ((m_spellInfo->AttributesEx6 & SPELL_ATTR_EX6_IGNORE_CCED_TARGETS) && !(*next)->CanFreeMove()))
                {
                    ++next;
                    continue;
                }
        ...
        case TARGET_RANDOM_FRIEND_CHAIN_IN_AREA:
        {
            ...
            while(t && next != tempTargetUnitMap.end())
            {
                ...
                if (!(m_spellInfo->AttributesEx2 & SPELL_ATTR_EX2_IGNORE_LOS) && !prev->IsWithinLOSInMap(*next, false))
                {
                    ++next;
                    continue;
                }
        ...
        case TARGET_CHAIN_DAMAGE:
        {
            ...
            else
            {
                ...
                while (t && next != tempTargetUnitMap.end())
                {
                    ...
                    if ((!(m_spellInfo->AttributesEx2 & SPELL_ATTR_EX2_IGNORE_LOS) && !prev->IsWithinLOSInMap(*next, false)) || ((m_spellInfo->AttributesEx6 & SPELL_ATTR_EX6_IGNORE_CCED_TARGETS) && !(*next)->CanFreeMove()))
                    {
                        ++next;
                        continue;
                    }
...
bool Spell::CheckTarget( Unit* target, SpellEffectIndex eff )
{
    ...
    switch(m_spellInfo->Effect[eff])
    {
        ...
        case SPELL_EFFECT_RESURRECT_NEW:
            ...
            if (target != m_caster && !target->IsVisibleTargetForSpell(m_caster, m_spellInfo, false))
            {
                if (!m_targets.getCorpseTargetGuid())
                    return false;
                ...
                if(!(m_spellInfo->AttributesEx2 & SPELL_ATTR_EX2_IGNORE_LOS) && !corpse->IsWithinLOSInMap(m_caster, false))
                    return false;
            }

...
void BattleGround::Update(uint32 diff)
{
    if (!GetPlayersSize())
    {
        ...
        // this will bg object, where any player entered
    ...
    if (GetStatus() == STATUS_WAIT_JOIN && GetPlayersSize())
    {
        ...
        // -- only Arathi Basin supported for now
        float deadly_Z = 0.0f;
...
bool BattleGround::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/)
{
    ...
    if (!go->Create(GetBgMap()->GenerateLocalLowGuid(HIGHGUID_GAMEOBJECT), entry, GetBgMap(), x, y, z, o, QuaternionData(rotation0, rotation1, rotation2, rotation3)))
    {

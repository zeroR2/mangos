    ...
    switch (damageInfo->m_spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            ...
            if (crit)
            {
                ...
                if (damageInfo->attackType != RANGED_ATTACK)
                    damageInfo->damage -= damageInfo->target->GetMeleeCritDamageReduction(reduction_affected_damage);
                else
                    damageInfo->damage -= damageInfo->target->GetRangedCritDamageReduction(reduction_affected_damage);
            }
            else
            {
                damageInfo->HitInfo &=  ~SPELL_HIT_TYPE_CRIT;
                ...
                damageInfo->damage -= damageInfo->target->GetSpellDamageReduction(CalcNotIgnoreDamageReduction(damageInfo));
            }
            break;
        }
        // Magical Attacks
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (crit)
            {
                ...
                // Resilience - reduce crit damage
                damageInfo->damage -= damageInfo->target->GetSpellCritDamageReduction(CalcNotIgnoreDamageReduction(damageInfo));
            }
            else
            {
                damageInfo->HitInfo &= ~SPELL_HIT_TYPE_CRIT;
...
bool Unit::IsVisibleTargetForSpell(WorldObject const* caster, SpellEntry const* spellInfo, bool strict) const
{
    ...
    // spell can hit stealth/invisible targets, just check for LoS
        ...
        return spellInfo->HasAttribute(SPELL_ATTR_EX2_IGNORE_LOS) ? true : caster->IsWithinLOSInMap(this, strict);
}

...
bool ChatHandler::HandleGameObjectAddCommand(char* args)
{
    ...
    pGameObj->SaveToDB(map->GetId(), plr->GetPhaseMaskForSpawn());
...
bool ChatHandler::HandleNpcAddCommand(char* args)
{
    ...
    pCreature->SaveToDB(map->GetId(), chr->GetPhaseMaskForSpawn());
...
bool ChatHandler::HandleNpcUnFollowCommand(char* /*args*/)
{
    ...
    FollowMovementGenerator<Creature> const* mgen
        = static_cast<FollowMovementGenerator<Creature> const*>((creature->GetMotionMaster()->CurrentMovementGenerator()));
...
bool ChatHandler::HandleWpModifyCommand(char* args)
{
    ...
    if (show == "add" && target)
    {
        ...
        wpCreature->SaveToDB(map->GetId(), chr->GetPhaseMaskForSpawn());
        // To call _LoadGoods(); _LoadQuests(); CreateTrainerSpells();
    ...
    if (show == "move" && target)
    {
        ...
        Map* map = chr->GetMap();
        {
            ...
            if (wpGuid != 0)
            {
                ...
                wpCreature2->SaveToDB(map->GetId(), chr->GetPhaseMaskForSpawn());
                // To call _LoadGoods(); _LoadQuests(); CreateTrainerSpells();
...
bool ChatHandler::HandleLearnAllCraftsCommand(char* /*args*/)
{
    for (uint32 i = 0; i < sSkillLineStore.GetNumRows(); ++i)
    {
        ...
        if (skillInfo->categoryId == SKILL_CATEGORY_PROFESSION || skillInfo->categoryId == SKILL_CATEGORY_SECONDARY)                         // only prof. with recipes have
        {
...
bool ChatHandler::HandleLearnAllRecipesCommand(char* args)
{
    ...
    for (uint32 i = 1; i < sSkillLineStore.GetNumRows(); ++i)
    {
        ...
        if (skillInfo->categoryId != SKILL_CATEGORY_PROFESSION &&
                skillInfo->categoryId != SKILL_CATEGORY_SECONDARY)                        // only prof with recipes have set
            continue;

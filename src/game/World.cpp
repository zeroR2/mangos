...
World::AddSession_ (WorldSession* s)
{
    ...
    packet << uint8(0);                        // 0 - normal, 1 - TBC, 2 - WotLK. Must be set in database manually for each account.
    s->SendPacket(&packet);
...
void World::AddQueuedSession(WorldSession* sess)
{
    ...
    packet << uint8(0);                                     // BillingPlanFlags
    packet << uint32(0);                                    // BillingTimeRested
    packet << uint8(0);                     // 0 - normal, 1 - TBC, must be set in database manually for each account

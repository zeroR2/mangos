...
void EventProcessor::Update(uint32 update_diff, uint32 p_time)
{
    ...
    for (EventList::size_type i = 0; i < m_events.size(); ++i)
    {
        if (m_aborting)
            break;

        BasicEventPtr Event = m_events[i];

        if (!Event || (Event->m_execTime > m_time && m_time - Event->m_execTime < DAY * IN_MILLISECONDS))
            continue;
